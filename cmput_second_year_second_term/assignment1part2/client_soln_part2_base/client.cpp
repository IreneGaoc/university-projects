...
void draw_path(int ,LonLat32* waypoints);
...
void draw_path(int len ,LonLat32* waypoints){
    
    if(len>0)
    {
      for (int i=1; i < len; i++) {
          // Convert lat/lon into x/y and draw them
          int32_t x1 = longitude_to_x(current_map_num, waypoints[i-1].lon) - screen_map_x;
          int32_t y1 = latitude_to_y(current_map_num, waypoints[i-1].lat) - screen_map_y;

          int32_t x2 = longitude_to_x(current_map_num, waypoints[i].lon) - screen_map_x;
          int32_t y2 = latitude_to_y(current_map_num,  waypoints[i].lat) - screen_map_y;

          tft.drawLine(x1, y1, x2, y2, ST7735_MAGENTA);
      }
  }
